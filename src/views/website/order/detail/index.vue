    ...
    <basic-container>
      <div class="flex align-center justify-between kp-block-title">
        ...
        <el-button
          ...
          :disabled="info.deliveredTimes == info.deliveryTimes"
          >添加</el-button
        >
        <el-tooltip v-else-if="beforedue" placement="top" content="未开始">
          <el-button type="primary" :disabled="beforedue">添加</el-button>
        </el-tooltip>
      ...
      >
        ...
        <template slot="menu" slot-scope="scope">
          <div class="flex center icons">
            ...
            <i v-else class="iconfont icontubiaozhizuomoban34 kp-opacity"></i>
            <el-tooltip
              class="item"
              effect="dark"
              content="编辑"
            ...
            <i v-else class="iconfont icontubiaozhizuomoban6 kp-opacity"></i>
            <el-tooltip
              class="item"
              effect="dark"
              content="删除"
            ...
            <i v-else class="iconfont icontubiaozhizuomoban22 kp-opacity"></i>
            <el-tooltip
              class="item"
              effect="dark"
              content="查看报告"
            ...
            >
              <i
                class="iconfont icontubiaozhizuomoban1"
                @click="toTaskReport(scope.row.taskCode)"
              ></i>
            </el-tooltip>
          </div>
        </template>
...
<script>
import { getOrderInfo, getOrderTaskList } from '@/api/new-media/order';
import { putDelTaskId } from '@/api/website/order';
import ResetTable from '@/mixins/resetTableHeight';
import orderInfo from '@/components/business-components/website/order/order-info';
import EditOrderDetailDialog from '@/components/dialog/website/order/edit-order-detail-dialog';
...
export default {
  ...
  computed: {
    ...
    beforedue() {
      return new Date(this.info.startTime).getTime() - Date.now() > 0;
    }
  },
  watch: {
    $route(to, from) {
  ...
  methods: {
    ...
    showEditDialog(row = {}) {
      ...
      if (
        ...
        this.info.deliveredTimes >= this.info.deliveryTimes
      ) {
        return;
      }
      let _self = this;
    ...
    // 关闭弹窗
    closeDialog() {
      this.isEditOrderDetailDialog = false;
      this.getData();
    },
    closeDialogN() {
      this.isEditOrderDetailDialog = false;
