  ...
  <div class="order">
        ...
        <el-row :gutter="10" type="flex" class="flex flex-wrap justify-between">
          ...
          <el-col :xs="24" :sm="24" :md="6" v-show="isPackUp">
            <el-form-item label="单位名称 :" prop="custName">
              <el-input v-model="orderForm.custName" placeholder="请输入" />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="6" v-show="isPackUp"></el-col>
          <el-col :xs="24" :sm="24" :md="6" v-show="isPackUp"></el-col>
          <el-col :xs="24" :sm="24" :md="6" v-show="isPackUp"></el-col>
          <el-col :xs="24" :sm="24" :md="6">
    ...
    <basic-container>
      ...
      >
        ...
        <template slot="dataPermission" slot-scope="scope">
          <div class="flex center">
            {{ scope.row.dataPermission == 1 ? '正式' : '试用' }}
          </div>
        </template>
...
export default {
  ...
  methods: {
    ...
    toDetail(row) {
      this.$router.push({
        path: `/website/order/detail/index`,
        query: { id: row.id }
      });
    },
    packUpHandle() {
