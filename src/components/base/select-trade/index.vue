...
<script>
import { getTradeListByParent, getTradeTree } from '@/api/common';
...
export default {
  ...
  data() {
    return {
      ...
      options: [],
      props: {
        label: this.lazy ? 'tradeName' : 'name',
        value: 'id',
        ...
        async lazyLoad(node, resolve) {
          ...
          let { code, data } = await getTradeListByParent(pid);
          if (code === 0) {
            data.forEach(item => {
  ...
  methods: {
    ...
    async getTreeData() {
      ...
      let { code, data } = await getTradeTree();
      this.loading = false;
      if (code === 0) {
        this.options = this.transData(data);
        setStore({ name: 'trade', content: this.options, type: 'session' });
      }
    },
    onChange(val) {
      this.self_value = val;
