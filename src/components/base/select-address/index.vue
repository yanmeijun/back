...
<script>
import { getDistrictListByParent, getDistrictTree } from '@/api/common';
    ...
    return {
      ...
      props: {
        ...
        async lazyLoad(node, resolve) {
          ...
          let { code, data } = await getDistrictListByParent(pid);
          if (code === 0) {
            data.forEach(item => {
  ...
  methods: {
    ...
    async getTreeData() {
      ...
      if (code === 0) {
        ...
        setStore({ name: 'address', content: this.options, type: 'session' });
      }
    },
    onChange(val) {
      this.self_value = val;
