      ...
      <div class="upload-info" v-if="fileName">
        <div class="flex err">
          ...
          <div v-if="isSuccess && isProgressEnd">
            <p>
              <span class="kp-color-success">读取成功</span>，共
              {{ resData.rightNum }} 条
            </p>
          </div>
          <div v-if="!isSuccess && isProgressEnd">
            ...
            <div v-else>
              <p>
                <span class="kp-color-error">读取失败</span>，正确
...
<script>
import { getFile, importFile } from '@/api/new-media/common';
...
export default {
  ...
  setup(props, { root, emit }) {
    ...
    const cDownErrorUrl = computed(() => {
      return `/newmedia-back/mediaAccountPool/downloadErrorMessage?uniqueness=${state.file}`;
    });
