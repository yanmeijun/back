        ...
        <el-row :gutter="10" type="flex" class="flex flex-wrap justify-between">
          ...
          <el-col :xs="24" :ms="24" :md="8" v-show="isPackUp">
            <el-form-item label="账号类型：" prop="accountTypeList">
              ...
              >
                <el-option
                  v-for="item in onFilterDictType(dicList.account_type)"
                  :key="item.label"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
...
import {
  getErrorList,
  getErrorSensitive,
  findByAccountNameOrUniqueCode
} from '@/api/new-media/data-review';
import {
  reactive,
  ref,
  toRefs,
...
export default {
  ...
  setup(props, { root }) {
    ...
    const cParams = computed(() => {
      return Object.assign(state.oneForm, { check: state.check });
    });
    ...
    //   );
    // });
    // const showErrorSensitive = computed(() => {
    //   return (
    //     state.oneForm.code ||
    ...
    const onRemoteMethod = async query => {
      ...
      state.infoOptions = data.map(ele => {
        return {
          label: ele,
          value: ele
        };
    ...
    const onLoad = page => {
      ...
      //   page.currentPage - state.prePage > 1 &&
      //   state.prePage <= 500
      // ) {
      //   root.$message.error('页面跨度超限');
      //   console.log('页面跨度超限', state.prePage);
      //   state.page = {
      //     ...page,
      //     currentPage: state.prePage
      //   };
      //   console.log(state.page);
      //   return;
      // }
      state.page = page;
      state.prePage = page.currentPage;
      onGetList();
    };
