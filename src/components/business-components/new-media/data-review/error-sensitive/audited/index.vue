<template>
  <div class="pending">
        ...
        <el-row :gutter="10" type="flex" class="flex flex-wrap justify-end">
          ...
          <el-col :xs="24" :ms="24" :md="8" v-show="isPackUp">
            <el-form-item label="账号类型：" prop="accountTypeList">
              <select-dict
                ...
                v-model="oneForm.accountTypeList"
                clearable
                :multiple="true"
                :type="`account_type`"
                :filter="onFilterDictType"
              ></select-dict>
            </el-form-item>
          </el-col>
          <el-col :xs="24" :ms="24" :md="8" v-show="isPackUp">
            <el-form-item label="账号信息：" prop="accountInfo">
              <GetAccountInfo :limit="5" v-model="oneForm.accountInfo" />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :ms="24" :md="8" v-show="isPackUp">
            <el-form-item label="问题类型：" prop="questionTypeList">
              ...
              >
                <el-option
                  v-for="item in dicList.sensitive_question_type"
                  :key="item.label"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
          ...
          <el-col :xs="24" :ms="24" :md="8" v-show="isPackUp">
            <el-form-item label="内容类型：" prop="contentType">
              <el-select
                clearable
                v-model="oneForm.contentType"
                placeholder="请选择"
              >
                <el-option
                  v-for="item in dicList.content_type"
                  :key="item.label"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
    ...
    >
      <basic-container v-if="cShowErrorSensitive">
        <ErrorSensitiveSearch
          :type="0"
          :line="true"
          :initData="cParams"
          @query="onGetErrorSensitive"
        />
        <ErrorSensitiveSearch
          :type="1"
          title="推荐修改"
    ...
    <basic-container>
      ...
      <div class="flex align-center justify-between kp-block-title">
        ...
        <div class="flex right">
          ...
          <el-button
            type="primary"
            :disabled="checkTableList.length === 0"
            @click="onShowDel('isSelected')"
            >删除</el-button
          >
          <ExportDropdown
            class="export-btn"
            :url="`/newmedia-back/mediaSensitiveDetail/exportData`"
            :idList="exportIdList"
            :args="exportArgs"
      ...
      >
        ...
        <!-- <template slot="errWordHeader" slot-scope="{ column }">
          <el-dropdown trigger="click" placement="bottom">
            <div class="el-dropdown-link">
              <span class="el-dropdown-link-txt">{{ column.title }}</span>
              <i class="el-icon-arrow-down el-icon--right"></i>
            </div>
            <el-dropdown-menu slot="dropdown">
              <el-dropdown-item
                ><i class="iconfont icontubiaozhizuomoban19 err-icon"></i
                >错敏词首字母正序</el-dropdown-item
              >
              <el-dropdown-item
                ><i class="el-icon-arrow-down el-icon--right err-icon"></i
                >错敏词首字母倒序</el-dropdown-item
              >
              <el-dropdown-item
                ><i class="el-icon-arrow-down el-icon--right err-icon"></i
                >监测时间倒序</el-dropdown-item
              >
            </el-dropdown-menu>
          </el-dropdown>
        </template> -->
        ...
        <template slot="homePage" slot-scope="scope">
          <el-link
            v-if="
              parseInt(scope.row.accountType) !== 101 && !!scope.row.homePage
            "
            type="primary"
            :underline="false"
          ...
          >
            <template>
              <div slot class="ewm-con kp-text-center">
                <img :src="scope.row.homePage" alt="" />
                微信扫一扫
              </div>
            </template>
          ...
          <span v-if="!scope.row.homePage">-</span>
        </template>
        ...
        <template slot="menu" slot-scope="scope">
          <div class="flex align-center icons">
            ...
            >
              <i
                class="iconfont icontubiaozhizuomoban55"
                :class="{ 'kp-opacity': scope.row.resultType === 4 }"
...
<script>
import { getErrorList, getErrorSensitive } from '@/api/new-media/data-review';
...
export default {
  ...
  setup(props, { root }) {
    ...
    const state = reactive({
      ...
      allShowColumn: ['index', 'selection', 'sensitiveWords'],
      showColumn: [
        'index',
        'selection',
        'sensitiveWords',
      ...
      page: {
        ...
        layouts: ['Total', 'Sizes', 'PrevPage', 'NextPage']
      },
      Ids: [],
      IdsParams: {},
      checkTableList: [],
    ...
    const cResultTypes = computed(() => {
      let arr =
        state.currentTab.prop === 'rightData'
          ? ['RIGHT', 'ALL']
          : ['WRONG', 'ALL'];

      return state.resultTypes
        .filter(ele => arr.includes(ele.description))
        .map(ele => {
          if (ele.value === '999') {
            ele.value = '';
          }
    ...
    watch(
      [() => state.oneForm.accountInfo, () => state.oneForm.codeTypeValInput],
      (
        [accountInfo, codeTypeValInput],
        [accountInfoOld, codeTypeValInputOld]
      ) => {
        if (!accountInfo.length && !codeTypeValInput) {
          state.cShowErrorSensitive = false;
          state.sensitiveWordsList = [];
          state.recommendUpdateList = [];

          if (
            accountInfoOld !== accountInfo ||
            (codeTypeValInput !== codeTypeValInputOld &&
              codeTypeValInputOld !== null)
          ) {
            onGetList();
          }
        }
    ...
    const cParams = computed(() => {
      const dataState = state.currentTab.prop === 'rightData' ? true : false;
      return Object.assign(state.oneForm, { check: state.check, dataState });
    });
    // const cShowErrorSensitive = computed(() => {
    //   return (
    //     state.oneForm.code ||
    //     state.oneForm.codeArray.length > 0 ||
    //     state.oneForm.info
    //   );
    // });
    onMounted(() => {
    ...
    const onGetList = async () => {
      ...
      const accountInfo = state.oneForm.accountInfo.join(',');
      ...
      // 正确 / 错误数据
      params.dataState = state.currentTab.prop === 'rightData' ? true : false;
    ...
    const onQueryList = () => {
      ...
      state.cShowErrorSensitive = false;
      nextTick(() => {
        state.cShowErrorSensitive =
          state.oneForm.codeTypeValInput ||
          state.oneForm.codeTypeVal.length > 0 ||
          state.oneForm.accountInfo.length > 0;
      });
      state.oneForm.recommendUpdateList = [];
      state.oneForm.sensitiveWordsList = [];
      onGetList();
    };
    ...
    // 高级筛选
    const onGetErrorSensitive = query => {
      if (query.type) {
        state.oneForm.recommendUpdateList = query.value;
      } else {
        state.oneForm.sensitiveWordsList = query.value;
      }
      onResetParams();
      onGetList();
    };
    ...
    const onShowDel = row => {
      ...
      IdsParams.ids =
        row === 'isSelected' && !row.id
          ...
          : [row.id];
      if (!IdsParams.ids || !IdsParams.ids.length) return;
      state.IdsParams = IdsParams;
    ...
    const onShowMisInfo = row => {
      ...
      // checkbox - 多选
      // if (row === 'isSelected' ) {
      //   const arr = [...new Set(state.checkTableList.map(ele => ele.result_type))];
      //   if (arr.length === 1) {
      //     IdsParams['detail'] = state.checkTableList[0].result_type_details;
      //   }
      // } else {
      //   IdsParams['detail'] = row.result_type_details;
      // }
      IdsParams.ids =
        row === 'isSelected' && !row.id
          ? state.checkTableList.map(ele => ele.id)
