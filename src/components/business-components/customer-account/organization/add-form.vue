  ...
  <div v-loading="loading">
    ...
    >
      ...
      <div>
        ...
        <el-row :gutter="10" class="row">
          <el-col :xs="24" :sm="24" :md="12">
            <el-form-item label="编码/用户名：" prop="custCode">
              <el-input
...
export default {
  ...
  data() {
    let validateCustCode = async (rule, value, callback) => {
      ...
      if (!/^[A-Za-z0-9]{6}$/.test(value)) {
        return callback(new Error('请输入6位字母/数字/字母数字组合'));
      }
      // 手动输入进行是否存在验证
    ...
    return {
      ...
      modelFm: {
        ...
        customParentRelationList: [
          {
            layerType: '-1',
            parentId: this.parent ? this.parent.id : 0, // 挂靠组织单位id
            parentCode: this.parent ? this.parent.custCode : '', // 挂靠组织单位标识码
            parentCodes: this.parent ? this.parent.parentCodes : '', // 挂靠组织单位标识码
            parentIds: this.parent ? this.parent.parentIds : '' // 挂靠组织单位标识码
          }
        ],
        sort: '', // 排序
        custCode: '', // 标识码
      ...
      rules: {
        ...
        custCode: [
          { required: true, message: '请输入', trigger: 'blur' },
          { validator: validateCustCode, trigger: 'blur' }
        ],
        custName: [
          { required: true, message: '请输入', trigger: 'blur' },
  ...
  methods: {
    ...
    validateCustCode() {
      // 触发验证
      this.$nextTick(() => {
        this.$refs['modelFm'].validateField(['custCode']);
      });
    }
  },
  created() {
