<template>
  <div>
      ...
      <el-row :gutter="10" type="flex" justify="end" class="flex flex-wrap">
        ...
        <el-col :xs="24" :sm="24" :md="8">
          <el-form-item prop="url" label-width="0">
            <el-input
              v-model="filter.model.url"
              class="el-input--select"
              placeholder="请输入url"
            >
              <el-select v-model="filter.model.ex_urlType" slot="prepend">
                <el-option label="主页URL" value="home"></el-option>
                <el-option label="跳转URL" value="jump"></el-option>
              </el-select>
            </el-input>
          </el-form-item>
        </el-col>
        <template v-if="filter.isMore">
          <el-col :xs="24" :sm="24" :md="8">
            <el-form-item label="地区：" prop="ex_addressValues">
              <select-address
                v-model="filter.model.ex_addressValues"
                clearable
              />
            </el-form-item>
          </el-col>
          <el-col :xs="24" :sm="24" :md="8">
            <el-form-item label="是否渲染：" prop="isFlag">
              <el-select v-model="filter.model.isFlag" placeholder="请选择">
                <el-option
          ...
          <el-col :xs="24" :sm="24" :md="8">
            <el-form-item label="状态：" prop="status">
              <el-select v-model="filter.model.status" placeholder="请选择">
                <el-option
                  v-for="item in dicList.status"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                >
          ...
          <el-col :xs="24" :sm="24" :md="8">
            <el-form-item label="网站类型：" prop="siteType">
              <el-select v-model="filter.model.siteType" placeholder="请选择">
                <el-option
    ...
    >
      <!-- 网站标识码 -->
      <template slot="custCode" slot-scope="{ row }">
        <!-- eslint-disable -->
        <el-link
          type="primary"
          class="word-ellipsis-1"
          :href="
            `https://jg.kaipuyun.cn/cloud_web/users_loginIndex.action?data=${row.custCodeEncrypt}${row.custPasswordEncrypt}`
          "
          target="_blank"
        >
      ...
      <template slot="menu" slot-scope="scope">
        <div class="flex center icons">
          ...
          >
            <i
              class="iconfont icontubiaozhizuomoban51"
              @click="onJumpLink(scope.row, 'https://www.baidu.com')"
            ></i>
          </el-tooltip>
          <el-tooltip
            class="item"
          ...
          >
            <i
              class="iconfont icontubiaozhizuomoban47"
              @click="onJumpLink(scope.row, 'https://www.baidu.com')"
            ></i>
          </el-tooltip> -->
        </div>
...
import AddDialog from '@/components/dialog/customer-account/website/add'; // 添加
import EditDialog from '@/components/dialog/customer-account/website/edit'; // 编辑
import TipInfoDialog from '@/components/dialog/tip-dialog/index'; // 提示弹框
import ChangeAffiliatedUnitDialog from '@/components/dialog/customer-account/website/change-affiliated-unit'; // 调整挂靠单位
...
export default {
    ...
    return {
      ...
      filter: {
        model: {
          url: '', // 主页url
          jumpUrl: '', // 跳转url
          ex_urlType: 'home', // home：主页url jump：跳转url
          ex_addressValues: [], // 地区
          provDistId: '', // 地区-省
          cityDistId: '', // 地区-市
          countyDistId: '', // 地区-县
  ...
  methods: {
    ...
    getSearchModel() {
      ...
      // 根据url查询类型组装url参数
      if (filterModel.ex_urlType === 'jump') {
        filterModel.jumpUrl = filterModel.url;
        filterModel.url = '';
      }
      // 地区处理
    ...
    onRowMediaAccountCountClick(row) {
      if (!row.mediaAccountCount || row.mediaAccountCount === '0') {
        return;
      }
