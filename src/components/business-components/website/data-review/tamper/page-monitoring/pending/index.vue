  ...
  <div class="pending">
    ...
    <basic-container>
      <div class="flex align-center justify-between kp-block-title">
        <span class="flex align-center name">页面监测</span>
...
<script>
import { getTamperPageList, setTamperPage } from '@/api/website/data-review';
import {
  reactive,
  ref,
  toRefs,
...
export default {
  ...
  setup(props, { root }) {
    ...
    const onGetList = async () => {
      ...
      state.loading = true;
      const { data, total } = await getTamperPageList(params);
      data.forEach(item => {
        // 地区处理
        item.location = [
          item.provDistName,
    ...
    const onSetResult = async resultType => {
      ...
      const { code, msg } = await setTamperPage(params);
      if (code) {
