  ...
  <div class="pending">
        ...
        <el-row :gutter="10" type="flex" class="flex flex-wrap justify-between">
          ...
          <el-col :xs="24" :ms="24" :md="8" v-show="isPackUp">
            <el-form-item label="问题类型：" prop="questionTypeList">
              ...
              >
                <el-option
                  v-for="item in dicList.homepage_question_type"
                  :key="item.label"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
    ...
    <basic-container>
      <div class="flex align-center justify-between kp-block-title">
        <span class="flex align-center name">DNS监测</span>
        <div class="flex right">
          ...
          <ExportDropdown
            class="export-btn"
            :url="`/newmedia-back/websiteTamperMonitor/exportData`"
            :idList="exportIdList"
            :args="exportArgs"
...
<script>
import { getTamperDNSList, setTamperDNS } from '@/api/website/data-review';
import {
  reactive,
  ref,
  toRefs,
...
export default {
  ...
  setup(props, { root }) {
    ...
    const onGetList = async () => {
      ...
      state.loading = true;
      const { data, total } = await getTamperDNSList(params);
      data.forEach(item => {
        // 地区处理
        item.location = [
          item.provDistName,
    ...
    const onSetResult = async resultType => {
      ...
      const { code, msg } = await setTamperDNS(params);
      if (code) {
