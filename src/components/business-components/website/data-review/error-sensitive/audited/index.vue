        ...
        <el-row :gutter="10" type="flex" class="flex flex-wrap justify-between">
          ...
          <el-col :xs="24" :ms="24" :md="8" v-show="isPackUp">
            <el-form-item label="内容类型：" prop="contentType">
              ...
              >
                <el-option
                  v-for="item in dicList.website_sensitive_page_type"
                  :key="item.label"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
...
<script>
import { getErrorList, getErrorSensitive } from '@/api/website/data-review';
...
import DeleteReasonDialog from '@/components/dialog/website/data-review/delete-reason-dialog';
import MisInfoReasonDialog from '@/components/dialog/website/data-review/misInfo-reason-dialog';
import EditReasonDialog from '@/components/dialog/website/data-review/edit-reason-dialog';
...
export default {
  ...
  setup(props, { root }) {
    ...
    watch(
      [() => state.oneForm.siteInfo, () => state.oneForm.codeTypeValInput],
      ([siteInfo, codeTypeValInput], [siteInfoOld, codeTypeValInputOld]) => {
        if (!siteInfo.length && !codeTypeValInput) {
          state.cShowErrorSensitive = false;
          state.sensitiveWordsList = [];
          state.recommendUpdateList = [];

          if (
            siteInfoOld !== siteInfo ||
    ...
    const onFilterResultType = (options = []) => {
      ...
      return options.filter(ele => arr.includes(ele.description));
    };
    const { dicList } = useGetDicList([
      'question_level',
      'account_type',
      'sensitive_question_type',
      'website_sensitive_page_type',
      'result_type'
    ]);
    // ---------------- END - Hook ----------------
    ...
    const onQueryList = () => {
      ...
      nextTick(() => {
        state.cShowErrorSensitive =
          ...
          state.oneForm.codeTypeVal.length > 0 ||
          state.oneForm.siteInfo.length > 0;
      });
