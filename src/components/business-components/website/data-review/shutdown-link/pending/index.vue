  ...
  <div class="pending">
    <basic-container :border="true" :mini="true" class="top-form">
      ...
      >
        <el-row :gutter="10" type="flex" class="flex flex-wrap justify-between">
          ...
          <el-col :xs="24" :ms="24" :md="8">
            <el-form-item class="no-margin" label="监测时间：" prop="time">
              <div class="flex">
                <el-date-picker
          ...
          <el-col :xs="24" :ms="24" :md="8" v-show="isPackUp">
            <el-form-item label="问题类型：" prop="questionTypeList">
              ...
              >
                <el-option
                  v-for="item in dicList.shutdown_question_type"
                  :key="item.label"
                  :label="item.label"
                  :value="item.value"
                ></el-option>
    ...
    <basic-container>
      <div class="flex align-center justify-between kp-block-title">
        <span class="flex align-center name">关停域名/链接</span>
        <div class="flex right">
          ...
          <ExportDropdown
            class="export-btn"
            :url="`/newmedia-back/websiteShutdownLink/exportData`"
            :idList="exportIdList"
            :args="exportArgs"
      ...
      >
        ...
        <template slot="abnormalSnapshot" slot-scope="scope">
          <div class="flex center icons">
            <el-link
              type="primary"
              target="_blank"
              :href="scope.row.abnormalSnapshot"
              :underline="false"
            >
              <i class="iconfont icontubiaozhizuomoban59"></i>
            </el-link>
...
export default {
  ...
  setup(props, { root }) {
    ...
    const onGetList = async () => {
      ...
      state.loading = true;
      const { data, total } = await getShutdownLinkList(params);
      data.forEach(item => {
        // 地区处理
        item.location = [
          item.provDistName,
    ...
    const onSetResult = async resultType => {
      ...
      const { code, msg } = await setShutdownLink(params);
      if (code) {
