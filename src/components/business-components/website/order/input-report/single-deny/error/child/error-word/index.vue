<template>
  <div>
    ...
    <div class="flex align-center justify-between kp-block-title">
      <span class="flex align-center name">严重表述错误-错敏信息</span>
    </div>
    <!--表格渲染-->
    <ex-table
      :data="table.data"
...
export default {
  ...
  methods: {
    ...
    transData(data = []) {
      data.forEach(item => {
        // 问题类型 存在多级情况
        item.ex_questionType = [item.questionType, item.secondQuestionType]
          .filter(item => item)
          .join('/');
        // 栏目由对象加工成集合形式
        item.ex_columns = [];
        Object.keys(item.channelInfo).forEach(k => {
          item.ex_columns.push({
            name: item.channelInfo[k],
            url: k
          });
        });
      });
    ...
    async getList() {
      ...
      let {
        ...
        data,
        total
      } = await inputReportApi.getWebsiteSensitiveDetail({
