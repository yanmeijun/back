  ...
  methods: {
    transData(data) {
      ...
      if (model.type.length) {
        let code = model.type[0];
        let isMiss = [17, 18].includes(code); // 是否缺失
    ...
    onMissingFactorChange(vals) {
      ...
      if (code === 17) {
        this.model.description = `办事指南要素缺失包括：${labels.join('，')}。`;
      }
      if (code === 18) {
        // eslint-disable-next-line
        this.model.description = `事项总数不足5个的，每个办理事项办事指南要素均缺失包括：${labels.join('，')}。`;
      }
    },
    async doSubmit() {
      // 问题类别
