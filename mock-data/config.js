...
export default {
  ...
  routes: {
    ...
    getInfo: {
      url: '/admin/user/info',
      json: 'mock-data/json/info.json'
    },
    //
    getMenu: {
      url: '/admin/menu',
      json: 'mock-data/json/menu.json'
    },
    // ------------------------------------------
    // 获取单个数据
    getSingleData: {
    ...
    getListByRenderFn: {
      ...
      renderFn: function(dataRes, req, res, ext) {
        ...
        dataRes.data.list = MOCK_LIB.getPageData(
          dataRes.data.list,
          req.body.pageIndex,
          req.body.pageSize,
          1
    ...
    getDataWithoutJson: {
      ...
      method: 'post',
      renderFn: function(dataRes, req, res, ext) {
        let content = ext.fs.readFileSync(
          ext.path.join(ext.path.resolve(''), 'mock-data/json/data.json')
        );
    ...
    getFile: {
      ...
      renderFn: function(dataRes, req, res, ext) {
        res
          .status(200)
          .sendFile(
            ext.path.join(ext.path.resolve(''), 'mock-data/json/identify.png')
          );
      }
    },
